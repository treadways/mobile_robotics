def propagationStep2(pose_init, time_init, time_new, left_vel, right_vel, radius, width, left_var, right_var):
    particle_set = []
    change_time = time_new - time_init

    for index in pose_init:
        index = np.array(index)
        left_vel_error = left_vel + np.random.normal(0, left_vel)
