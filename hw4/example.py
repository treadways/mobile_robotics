...
#         mlab.points3d(A[:, 0], A[:, 1], A[:, 2], color=(
#             0, 1, 0), scale_factor=0.5, opacity=0.5)
#         mlab.points3d(B[:, 0], B[:, 1], B[:, 2], color=(
#             0, 0, 1), scale_factor=0.5, opacity=0.5)
...
def run_demo(num_points, bc_min, bc_max, range_rad, trans_max, noise_sd):
    ...
    transformed = apply_random_transform(
        points_A, range_rad, trans_max, noise_sd)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        'num_points', help="Number of test points to generate.")
    arg_parser.add_argument(
        'bc_min', help="Min x,y,z value for genrated points(within bounding cube).")
    arg_parser.add_argument(
        'bc_max', help="Max x,y,z value for genrated points(within bounding cube).")
    arg_parser.add_argument(
        '--range_radians', help="Range of randomised Euler Angles. [0, 2pi)")
    arg_parser.add_argument(
        '--trans_max', help="Max value for random translation.")
    arg_parser.add_argument(
        '--noise_sd', help="Random noise Standard Deviation for transformed points.")
    ...
    run_demo(int(args.num_points), float(args.bc_min), float(
        args.bc_max), range_rad, trans_max, noise_sd)
